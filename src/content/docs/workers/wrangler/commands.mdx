...
* [`workflows`](#workflows) - Configure and register [Workflows](/workflows/).
...
List Workflows in this account.
...
* `NAME` - `string` (required)
  * The name of the Workflow to inspect.
...
```txt
wrangler workflows trigger <name>
```

* `NAME` - `string` (required)
  * The name of the Workflow to trigger.
* `--params` - `string` (optional)
  * Parameters to pass to the Workflow instance in JSON format.

For example, to trigger a Workflow and provide a payload (params) that are passed to your Workflow's `run` method:

```sh
wrangler workflows trigger my-workflow --params='{"some_data":{"payload":"abc123def456"},"created_at":"123456"}'
```

### `instances`

Inspect and interact with specific instances of a Workflow.
